...
for i in range(qntd_horas):
    horas_disponiveis = (input("Digite o horário disponivel: "))
...
for i in range(len(horas_disponiveis)):
    ...
    for i, horas_disponiveis in enumerate(horarios):
        print("{} - {}".format(i, horas_disponiveis))
    ...
    if 1 <= escolher_horario <= len(horas_disponiveis):
        ...
        print("O horário {} foi agendado com sucesso!".format(horario_selecionado))
